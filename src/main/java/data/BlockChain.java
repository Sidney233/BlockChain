...
public class BlockChain {
    ...
    private Transaction[] genesisTransactions(Account[] accounts) {
        ...
        for (int i = 0; i < accounts.length; ++i) {
            outUtxos[i] = new UTXO(accounts[i].getWalletAddress(), MiniChainConfig.INT_AMOUNT, accounts[i].getPublicKey());
        }
    ...
    public UTXO[] getTrueUtxos(String walletAddress) {
        ...
        for (Block block : chain) {
            ...
            for (Transaction transaction : transactions) {
                UTXO[] inUtxos = transaction.getInUTXO();
                UTXO[] outUtxos = transaction.getOutUTXO();
